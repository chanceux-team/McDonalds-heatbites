...
const { parseMenuTitle } = useNavbarParser();
...
const titlesText = computed<string[]>(() =>
  (
    awesome?.layout?.welcome?.title || awesome?.name || 'Nuxt&nbsp;3 Awesome Starter'
  )
    .replaceAll('&nbsp;', '[space]')
    .split(' ')
    ...
    <LayoutPageSection class="flex-1 flex">
      <div class="flex-1 flex flex-col items-center justify-center">
        ...
        <div class="px-4 mt-6 text-center max-w-[500px] md:max-w-[600px]">
          {{
            awesome?.description || 'a starter template for Nuxt 3 with minimalist themes design, built in components, drawer & menus, and more.'
          }}
        </div>
        <div
          v-if="showAlert"
